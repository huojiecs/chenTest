...
var commandDispatch = require('./commands/commandDispatch');
...
handler.StartRestfulServer = function () {
    ...
    function respond_post_command(req, res, next) {
        ...
        Q.resolve()
            .then(function () {
                ...
                req.data_packet = JSON.parse(urlencode.decode(req.body.replace('data_packet=', ''), 'utf8'));
                //logger.debug('recv post idip: %j', req.data_packet);
    ...
    function respond_command(req, res, next) {
        ...
        /**　消息分发  及参数检查 */
        return commandDispatch.beforeDispatch(req.data_packet);
    }
    ...
    server.listen(8130, curServer.idipHost, function () {
